...
function Profile() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:3030/admin/user-data', { withCredentials: true });
        setFormData({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.patch('http://localhost:3030/admin/update', formData, { withCredentials: true });
      navigate('/dashboard');
    } catch (error) {
