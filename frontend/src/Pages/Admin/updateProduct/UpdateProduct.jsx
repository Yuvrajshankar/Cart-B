...
function UpdateProduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`http://localhost:3030/product/${id}`, { withCredentials: true });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.patch(`http://localhost:3030/product/update/${id}`, product, { withCredentials: true });
      navigate("/dashboard");
    } catch (err) {
