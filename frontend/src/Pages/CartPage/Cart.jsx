...
function Cart() {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const response = await axios.get('http://localhost:3030/user/user-data', { withCredentials: true });
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const cartDetailsPromises = cartData.map(async (cartItem) => {
          const response = await axios.get(`http://localhost:3030/products/${cartItem.productId}`, { withCredentials: true });
        ...
        const boughtListDetailsPromises = boughtList.map(async (boughtItem) => {
          const response = await axios.get(`http://localhost:3030/products/${boughtItem.productId}`, { withCredentials: true });
  ...
  const handleBuy = async (productId) => {
    try {
      const response = await axios.post(`http://localhost:3030/products/buy/${productId}`, {}, { withCredentials: true });
            ...
            {totalProducts === 0 ? (
              ...
              cartData.map((product, index) => (
                <Product
                  ...
                  img={`http://localhost:3030/images/${productDetails[index]?.image}`}
            ...
            <>
              ...
              {boughtList.map((product, index) => (
                <BoughtProduct
                  key={product.productId}
                  productId={product.productId}
                  img={`http://localhost:3030/images/${BoughtListProductDetails[index]?.image}`}
